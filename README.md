...
## Noise in Point Cloud Clustering

In the context of point cloud clustering, **noise** refers to points that do not belong to any meaningful structure or cluster.

Noise can appear due to:

* sensor inaccuracies
* reflections on shiny or distant surfaces
* sparse measurements at long range
* moving objects or environmental interference

Different algorithms handle noise differently:

* **K-Means** does not explicitly model noise; noisy points are forced into the nearest cluster.
* **Euclidean Clustering** may incorrectly group isolated points if the distance threshold is too large.
* **DBSCAN** explicitly labels noise points as outliers when they do not satisfy density conditions.
* **FCC** naturally filters noise by relying on local channel connectivity and spatial consistency, making it well suited for LiDAR data.

Correct handling of noise is crucial for obtaining reliable clustering results, especially in real-world scenarios.
...
* **Synthetic datasets**
  ...
  * simple shapes, varying densities, and controlled noise

* **Real dataset**

  * used exclusively for FCC
  * contains complex structures, occlusions, and sensor-specific noise
...
To run fcc on artificial dataset
```bash
./fcc2.sh
```
...
### Visualizing FCC Clustering on Real Data

To visualize how clusters are formed on the real LiDAR dataset, use the following command:

```bash
python3 plot_full_kitty.py ../data/lidar_stream.txt --save scena_finala.png
```

This command generates a visual representation of the clustered point cloud, allowing inspection of how FCC groups points based on sensor topology and spatial continuity.
...
<img width="400" alt="fccoriginal" src="https://github.com/user-attachments/assets/8f404f3f-b71b-452f-874c-ed5780f1944b" />
<img width="400" alt="fccresult" src="https://github.com/user-attachments/assets/247d737b-3736-4524-b076-f50271eab058" />
